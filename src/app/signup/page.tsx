...
import { Lock, Mail, ArrowRight, User, Building, AlertTriangle, CheckCircle2 } from "lucide-react";
...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!fullName.trim() || !orgName.trim() || !email.trim() || !password || !confirmPassword) {
    ...
    if (isDemoMode) {
      if (typeof window !== "undefined") {
        localStorage.setItem("nexusops_demo_signup_name", fullName.trim());
        localStorage.setItem("nexusops_demo_signup_org", orgName.trim());
        localStorage.setItem("nexusops_demo_signup_email", email.trim());
      }
    ...
    try {
      // Create Supabase Auth user with metadata options
      ...
      if (data.user) {
        // Try creating the profile record immediately
        try {
          ...
          console.error("Direct profile insertion error (it will fallback on mount):", dbErr);
        }
        ...
        if (data.session) {
          ...
          setSuccessMsg("Sign up successful! Please check your email inbox to verify your account.");
          setLoading(false);
        }
      }
    } catch (err: any) {
  ...
  return (
    <div className="h-screen w-screen bg-[#050505] text-[#f4f4f5] flex items-center justify-center font-sans select-none relative overflow-hidden">
      ...
      <motion.div
        ...
        className="bg-[#0b0b0c]/80 border border-zinc-900 rounded-3xl p-8 shadow-2xl shadow-black/80 max-w-sm w-full flex flex-col gap-5 backdrop-blur-md relative z-10 hover:border-zinc-800 transition-all duration-300"
      >
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-xs mt-1">Register your organization on NEXUSOPS</p>
          </div>
        </div>
        ...
        {isDemoMode && (
          <div className="text-[10px] text-purple-400 bg-purple-500/10 border border-purple-500/20 rounded-xl p-2.5 text-center font-medium leading-normal">
            ⚡ Offline Demo Mode Active. Registered account will be simulated locally.
          </div>
        )}
        ...
        <form onSubmit={handleSignup} className="flex flex-col gap-3.5">
          <div className="flex flex-col gap-1">
            <label className="text-[10px] text-zinc-500 uppercase tracking-wider font-bold">Full Name</label>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[10px] text-zinc-500 uppercase tracking-wider font-bold">Organization Name</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="Alpha Gaming Syndicate"
                className="w-full bg-[#050505] border border-zinc-900 focus:border-[#7C3AED]/40 rounded-xl pl-10 pr-4 py-2.5 text-xs text-zinc-300 placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-[#7C3AED]/20 transition-all font-sans"
                suppressHydrationWarning
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] text-zinc-500 uppercase tracking-wider font-bold">Confirm</label>
              <div className="relative">
                <Lock className="absolute left-3.5 top-3 w-4 h-4 text-zinc-500" />
                <input
                  type="password"
